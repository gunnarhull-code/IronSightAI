...
- **Date**: 2026-07-31
- **Decision**: Live MVP scope is a capture-and-review Quick Appraisal a sales rep can complete in under two minutes offline, producing a trustworthy inspection package with **no dollar valuation**. Optional advisory AI may suggest recognition, condition/damage, field values, and missing information; humans review, correct, and explicitly confirm. AI never silently overwrites data, is never final authority, and never blocks inspections. Preserve a provider-agnostic `AIService` architecture. **V2** is a clean shareable summary image/PDF (not a screenshot), prefilled email draft, review-before-send, and native device sharing. **Later** covers portal, collaboration/chat, manager approval, automated email, historical equipment info, recon costs/additions, and pricing/valuation recommendations. Docs-only Work Item #18; PR #14 and PR #15 remain frozen.
- **Reasoning**: Aligns governing specifications with the founder-approved product focus: speed and a trustworthy offline package first; sharing next; valuation and collaboration later. Restores the under-two-minute measuring stick while keeping AI assistive rather than authoritative.
- **Alternatives Considered**: (a) Leave pre-#18 specs unchanged (zero AI in V1; PDF/share in V1; “just a few minutes”) — rejected, contradicts the approved direction. (b) Expand frozen implementation PRs #14/#15 to match — rejected, scopes remain frozen. (c) Rewrite every historical supporting doc (`01`–`14`) in place — rejected as unnecessary for this Work Item; `docs/15` §0 is live authority.
- **Status**: Approved (supersedes live-scope portions of ADR-007’s “zero AI in V1”, ADR-009’s softened timing wording for the default path, and ADR-014’s “native share in V1” timing; those ADRs remain historical record)
- **Related Documents**: GitHub Issue #18; `docs/15-final-product-specification.md` §0 / §16 / §19; `docs/00-ironsight-constitution.md`; `docs/13-roadmap.md`; `docs/18-implementation-ready-report.md`; `README.md`
