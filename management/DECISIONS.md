...
## ADR-018: Capture-and-Review MVP Direction (Work Item #18)

- **Date**: 2026-07-31
- **Decision**: Live MVP scope is **Quick Appraisal only**: a sales rep completes a useful inspection package in under two minutes offline, with **no dollar valuation** and **no Detailed Inspection UI** (Detailed Inspection is Later; preserve the shared checklist/`parent_item_id` data model for a future add without rewrite). **Minimum capture set:** required photos (front-left, rear-right, serial/data plate, hour-meter/dashboard); continuous guided walkaround video as evidence (**not** AI-analyzed in MVP); type/make/model; serial or “not found”; hours or “unknown”; Quick Condition ratings; optional year/notes/extra damage photos. **Timing:** Start Quick Appraisal → confirm complete; excludes login, first-time company onboarding, sync, and sharing; validate on mid-range phone in field conditions; if over two minutes, reduce required work. **AI:** on-device OCR for serial/hours; optional cloud photo recognition/autofill when connected only via backend provider-agnostic `AIService` (Flutter/domain never call a vendor directly); manual path always works; vendor selection is a later benchmark Work Item. **MVP plumbing:** minimal company setup/context, company-scoped inspection list, reopen mutable draft, discard draft with confirmation. **V2:** professional PDF generated server-side, cached/downloaded locally, attached through a reviewed native email/share draft; separate summary-image renderer is Later. **Later** also covers portal, collaboration/chat, manager approval, automated email, historical equipment info, recon, and pricing/valuation. Docs-only Work Item #18; PR #14 and PR #15 remain frozen. Former `docs/15` §19 open questions are resolved (see §19 confirmation table).
- **Reasoning**: Aligns governing specifications with founder-approved product focus and the five explicit confirmations that closed the post-#18 open questions: speed and a trustworthy offline Quick Appraisal first; professional PDF sharing next; Detailed Inspection, summary image, portal/collaboration, and valuation later — without abandoning the shared data model needed for depth later.
- **Alternatives Considered**: (a) Leave pre-#18 specs unchanged (zero AI in V1; PDF/share in V1; Detailed Inspection in V1; “just a few minutes”) — rejected. (b) Keep Detailed Inspection as optional MVP depth — rejected by founder confirmation; move to Later. (c) Hardcode a cloud AI vendor in the product spec — rejected; benchmark later. (d) Ship summary-image in V2 alongside PDF — deferred to Later unless pilot feedback proves it necessary. (e) Expand frozen implementation PRs #14/#15 — rejected, scopes remain frozen.
- **Status**: Approved (supersedes live-scope portions of ADR-007’s “zero AI in V1”, ADR-009’s “Detailed Inspection ships in V1” / softened timing wording, and ADR-014’s “native share in V1” timing; those ADRs remain historical record)
- **Related Documents**: GitHub Issue #18; Draft PR #19; `docs/15-final-product-specification.md` §0 / §16 / §19; `docs/00-ironsight-constitution.md`; `docs/13-roadmap.md`; `docs/18-implementation-ready-report.md`; `README.md`
